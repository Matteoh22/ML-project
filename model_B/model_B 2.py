...
OUT_DIR  = Path("./outputs/model_B_compact_scientific"); OUT_DIR.mkdir(parents=True, exist_ok=True)
...
counts = np.bincount(np.array(y_train), minlength=len(class_names))
dev = np.max(np.abs(counts - counts.mean())/counts.mean())
class_weight = None
if dev > 0.15:
    w = compute_class_weight("balanced", classes=np.arange(len(class_names)), y=np.array(y_train))
